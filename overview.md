...
This portfolio is designed to represent the pinnacle of my current technical ability
across frontend, backend, systems, and machine learning.
...
The goal is to make my skills obvious without requiring explanations, accounts, or setup
from the viewer.
...
Users may click UI elements (desktop-style)
...
About: name, experience, tech stack
Projects: interactive directory view
Contact: links and communication info
The Projects view displays repositories in a grid layout:

Each project is a card with:
Title (top-left)
Technology tags (above or near title)
Short ML-generated summary
Optional animated GIF as the background
Cards use rounded corners and semi-transparent overlays
to maintain text readability over visuals
The frontend does not perform heavy data processing and relies entirely on APIs.

Go Service (GitHub Ingestion Layer)

A dedicated Go service is responsible for interacting with the GitHub API.

Responsibilities:

Fetch repository metadata using:
...
Demonstrate concurrency and systems-level thinking
Keep the Django core focused and maintainable
...
Django is intentionally not responsible for:
...
Model outputs are deterministic and debuggable.

Data Flow Summary

Go service fetches and caches GitHub repository data
Django consumes structured data
README text is passed through the ML pipeline
Django stores and serves enriched project data
React frontend renders the final interactive UI
Design Principles
